<?php
/**
 * Plugin Name:       Kunde Beskeder (Spjaet)
 * Plugin URI:        https://spjaet.dk
 * Description:       Videresender interne beskeder til kunder på deres "Min Konto" side.
 * Version:           1.3.7
...
/**
 * ========================================================================
 * ADMIN-FUNKTIONER (CPT, METABOX, KOLONNER)
 * ========================================================================
 */

/**
 * TRIN 2: Registrer Custom Post Type 'kunde_besked'
 */
function kb_opret_cpt() {
    $labels = array(
        'name'                  => _x( 'Kunde Beskeder', 'Post Type General Name', 'kunde-beskeder' ),
        'singular_name'         => _x( 'Kunde Besked', 'Post Type Singular Name', 'kunde-beskeder' ),
        'menu_name'             => __( 'Kunde Beskeder', 'kunde-beskeder' ),
        'name_admin_bar'        => __( 'Kunde Besked', 'kunde-beskeder' ),
        'all_items'             => __( 'Alle Beskeder', 'kunde-beskeder' ),
        'add_new_item'          => __( 'Opret Ny Besked', 'kunde-beskeder' ),
        'add_new'               => __( 'Opret Ny', 'kunde-beskeder' ),
        'new_item'              => __( 'Ny Besked', 'kunde-beskeder' ),
        'edit_item'             => __( 'Rediger Besked', 'kunde-beskeder' ),
        'update_item'           => __( 'Opdater Besked', 'kunde-beskeder' ),
    );
    $args = array(
        'label'                 => __( 'Kunde Besked', 'kunde-beskeder' ),
        'description'           => __( 'Interne beskeder til kunder', 'kunde-beskeder' ),
        'labels'                => $labels,
...
/**
 * TRIN 3: Opret Metabox til Målretning (SIMPEL VERSION)
 */

function kb_tilfoej_metabox() {
    add_meta_box(
        'kb_maalretning_metabox', 'Besked Målretning', 'kb_metabox_html_output', 'kunde_besked', 'side', 'high'
    );
}
...
function kb_metabox_html_output( $post ) {
    ...
    <div class="kb-fields-wrapper">
        <p>Vælg hvordan denne besked skal målrettes.</p>
        <div>
            <label><input type="radio" class="kb_maal_type_radio" name="kb_maal_type" value="status" <?php checked( $maal_type, 'status' ); ?>> Målret efter Abonnement-status</label>
            <label><input type="radio" class="kb_maal_type_radio" name="kb_maal_type" value="produkt" <?php checked( $maal_type, 'produkt' ); ?>> Målret efter Produkt (Hold) ID</label>
            <label><input type="radio" class="kb_maal_type_radio" name="kb_maal_type" value="kunde" <?php checked( $maal_type, 'kunde' ); ?>> Målret specifik kunde ID</label>
        </div>
        
        <div id="kb-status-group" class="kb-field-group">
            <p>Vis til kunder med status:</p>
            <label><input type="checkbox" name="kb_statusser[]" value="wc-active" <?php checked( in_array( 'wc-active', $statusser ) ); ?>> Aktiv</label>
            <label><input type="checkbox" name="kb_statusser[]" value="wc-on-hold" <?php checked( in_array( 'wc-on-hold', $statusser ) ); ?>> Parkeret (On-hold)</label>
            <label><input type="checkbox" name="kb_statusser[]" value="wc-pending" <?php checked( in_array( 'wc-pending', $statusser ) ); ?>> Afventende (Pending)</label>
        </div>
        
        <div id="kb-produkt-group" class="kb-field-group">
            <p>Vis til kunder på dette hold:</p>
            <label for="kb_produkt_id_input">Produkt ID:</label>
            <input type="number" id="kb_produkt_id_input" name="kb_produkt_id" value="<?php echo esc_attr( $produkt_id ); ?>" placeholder="Find produktets ID under 'Produkter'...">
        </div>
        
        <div id="kb-kunde-group" class="kb-field-group">
            <p>Vis kun til denne kunde:</p>
            <label for="kb_kunde_id_input">Kunde ID:</label>
            <input type="number" id="kb_kunde_id_input" name="kb_kunde_id" value="<?php echo esc_attr( $kunde_id ); ?>" placeholder="Find kundens ID under 'Brugere'...">
        </div>
    </div>
    <?php
}
...
/**
 * TRIN 3b: Simpel JS til at vise/skjule felter
 */
...
/**
 * TRIN 3.5: Tilføj Admin Kolonner
 */
...
function kb_tilfoej_admin_kolonne_header( $columns ) {
    ...
    foreach ( $columns as $key => $title ) {
        ...
        if ( $key === 'title' ) {
            $new_columns['kb_maalretning'] = __( 'Målretning', 'kunde-beskeder' );
        }
    }
    return $new_columns;
}
        ...
        switch ( $maal_type ) {
            case 'status':
                ...
                if ( empty( $statusser ) || ! is_array( $statusser ) ) {
                    echo '<em>Status: Ingen valgt</em>';
                } else {
                    $pæne_navne = array();
                    foreach ( $statusser as $status ) {
                        if ( $status === 'wc-active' ) $pæne_navne[] = 'Aktiv';
                        elseif ( $status === 'wc-on-hold' ) $pæne_navne[] = 'Parkeret';
                        elseif ( $status === 'wc-pending' ) $pæne_navne[] = 'Afventende';
                        else $pæne_navne[] = ucfirst( $status );
                    }
                    echo '<strong>Status:</strong> ' . esc_html( implode( ', ', $pæne_navne ) );
                }
                break;
            ...
            case 'produkt':
                ...
                if ( empty($produkt_titel) ) $produkt_titel = 'Ukendt Produkt';
                echo '<strong>Produkt:</strong> ' . esc_html( $produkt_titel ) . ' (ID: ' . $produkt_id . ')';
                break;
            ...
            case 'kunde':
                ...
                if ( $kunde ) {
                    echo '<strong>Kunde:</strong> ' . esc_html( $kunde->display_name ) . ' (ID: ' . $kunde_id . ')';
                } else {
                    echo '<strong>Kunde:</strong> <em>Ukendt (ID: ' . $kunde_id . ')</em>';
                }
                break;
            
            default:
                echo '<em>Ikke specificeret</em>';
                break;
        }
    }
}
...
/**
 * TRIN 3.6: Tilføj Opt-in felt til Admin Brugerprofil
 */
 
function kb_vis_admin_notits_felt( $user ) {
    $meta_value = get_user_meta( $user->ID, '_kb_email_notits', true );
    $er_checked = ( $meta_value === '0' ) ? 0 : 1; // Default til 'yes' (1)
    ?>
    <h3><?php _e('Kunde Beskeder Notifikationer', 'kunde-beskeder'); ?></h3>
    <table class="form-table">
        <tr>
            <th><label for="_kb_email_notits"><?php _e('Modtag E-mails', 'kunde-beskeder'); ?></label></th>
            <td>
                <input type="checkbox" name="_kb_email_notits" id="_kb_email_notits" value="1" <?php checked( $er_checked, 1 ); ?>>
                <span class="description"><?php _e('Send brugeren en e-mail-notits, når der er en ny besked.', 'kunde-beskeder'); ?></span>
            </td>
        </tr>
    </table>
    <?php
...
/**
 * ========================================================================
 * FRONTEND-FUNKTIONER (MIN KONTO)
 * ========================================================================
 */

/**
 * 4.1: Tilføj 'beskeder' til listen over 'Min Konto' endpoints.
 */
function kb_tilfoej_endpoint() {
    add_rewrite_endpoint( 'beskeder', EP_PAGES );
}
add_action( 'init', 'kb_tilfoej_endpoint' );

/**
 * 4.2: Tilføj det nye menupunkt "Beskeder" til "Min Konto"-menuen.
 */
function kb_tilfoej_menu_link( $items ) {
    if ( isset( $items['customer-logout'] ) ) {
        $logout_link = $items['customer-logout'];
        unset( $items['customer-logout'] );
        $items['beskeder'] = __( 'Beskeder', 'kunde-beskeder' );
...
/**
 * 4.3: Den vigtigste funktion: Indlæs og vis indholdet for "Beskeder"-fanen.
 */
function kb_vis_beskeder_indhold() {
    
    echo '<h2>' . esc_html__( 'Dine beskeder', 'kunde-beskeder' ) . '</h2>';

    $matchende_beskeder = kb_hent_matchende_beskeder();

    if ( empty( $matchende_beskeder ) ) {
        echo '<p>' . esc_html__( 'Du har ingen nye beskeder.', 'kunde-beskeder' ) . '</p>';
    } else {
        echo '<ul class="kb-besked-liste">';
        
        $count = 0; // Tæller til unikke ID'er
        foreach ( $matchende_beskeder as $besked_post ) {
            ...
            $limit_ord = 55; // Ca. 55 ord før "Læs mere"
            
            // **RETTET v1.3.7**: Brug wp_trim_words, da den er sikker og bevarer <p> tags
            $kort_indhold = wp_trim_words( $indhold, $limit_ord, '' ); // Tom 'more'
            
            // Tjek om teksten rent faktisk BLEV afkortet
            if ( $kort_indhold !== $indhold ) {
                 $skal_foldes_ud = true;
                 $kort_indhold .= '...'; // Tilføj selv '...'
            }
            ...
            if ( $skal_foldes_ud ) {
                ...
                echo '<br><a href="#kb-besked-fuld-' . $count . '" class="kb-toggle-link" data-target-kort="kb-besked-kort-' . $count . '" data-target-fuld="kb-besked-fuld-' . $count . '">Læs mere...</a>';
                echo '</div>';
                
                echo '<div class="kb-besked-indhold kb-besked-fuld" id="kb-besked-fuld-' . $count . '" style="display:none;">';
                echo wp_kses_post( $indhold );
                echo '<br><a href="#kb-besked-kort-' . $count . '" class="kb-toggle-link" data-target-kort="kb-besked-kort-' . $count . '" data-target-fuld="kb-besked-fuld-' . $count . '">Vis mindre</a>';
                echo '</div>';
            } else {
...
add_action( 'woocommerce_account_beskeder_endpoint', 'kb_vis_beskeder_indhold' );


/**
 * 4.4: KERNE-LOGIK: Hent de beskeder, der matcher den nuværende bruger.
 */
...
/**
 * 5.1: Tilføj "Opt-in" checkbox til "Kontooplysninger" siden
 */
function kb_tilfoej_notits_felt() {
    ...
    woocommerce_form_field(
        ...
        array(
            ...
            'label'       => __( 'E-mail Notifikationer', 'kunde-beskeder' ),
            'description' => __( 'Send mig en e-mail-notits, når der er nye beskeder.', 'kunde-beskeder' ),
        ),
        $er_checked 
    );
}
...
/**
 * 5.3: Tjek om der skal sendes e-mails, NÅR EN POST ER GEMT (inkl. meta)
 *
 * **RETTET v1.3.7**: Vi bruger 'wp_after_insert_post'
 * Denne krog garanterer, at post OG meta-data er gemt.
 */
function kb_tjek_for_ny_besked( $post_id, $post, $update, $post_before ) {
    
    // Tjek at det er den rigtige post type
    if ( $post->post_type !== 'kunde_besked' ) {
        return;
    }
    
    // Tjek at det er en 'publish' status
    if ( $post->post_status !== 'publish' ) {
        return;
    }
    
    // Tjek at det er en NY post (ikke en opdatering)
    // '$update' er 'false' første gang en post gemmes
    if ( $update === true ) {
        // Tjek om det var en overgang til 'publish'
        if ( $post_before->post_status === 'publish' ) {
            // Det var allerede 'publish', så det er en opdatering. Stop.
            return;
        }
        // Ellers er det en overgang (f.eks. Kladde -> Publish), hvilket er OK.
    }

    // OK! Dette er en helt ny besked, der bliver publiceret.
    // Vi sætter den i kø (asynkron), så admin ikke skal vente.
    wp_schedule_single_event( time() + 5, 'kb_async_send_notitser', array( $post_id ) );
}
// **RETTET v1.3.7**: Accepterer nu 4 argumenter
add_action( 'wp_after_insert_post', 'kb_tjek_for_ny_besked', 20, 4 );


/**
 * 5.4: Hovedfunktion til at finde modtagere og sende e-mails
 */
...
function kb_koer_notits_udsendelse( $post_id ) {
    ...
    // **RETTET v1.3.6**: Læs fra get_post_meta(), nu hvor vi er sikre på, det er gemt.
    $maal_type = get_post_meta( $post_id, '_kb_maal_type', true );
    
    $modtager_ids = array(); 

    // 1. FIND ALLE MATCHENDE KUNDER
    if ( $maal_type === 'kunde' ) {
        $kunde_id = absint( get_post_meta( $post_id, '_kb_kunde_id', true ) );
        if ( $kunde_id > 0 ) {
    ...
    // 2. FILTRER MODTAGERE OG SEND E-MAILS
    if ( ! empty( $modtager_ids ) ) {
        
        $emne = get_bloginfo( 'name' ) . ': Du har en ny besked';
        ...
        foreach ( array_unique( $modtager_ids ) as $kunde_id ) {
            ...
            $vil_modtage = ( $opt_in_value === '0' ) ? false : true; // Default til 'yes'
            ...
            if ( $vil_modtage ) {
                ...
                if ( $kunde ) {
                    ...
                    $email_body  = "Hej " . $modtager_navn . ",\n\n";
                    $email_body .= "Du har modtaget en ny besked på " . get_bloginfo( 'name' ) . ".\n\n";
                    $email_body .= "Emne: \"" . $besked_titel . "\"\n\n";
                    $email_body .= "Log venligst ind på din 'Min Konto' side for at læse beskeden:\n";
                    $email_body .= $login_url . "\n\n";
                    $email_body .= "Med venlig hilsen,\n";
                    $email_body .= "Spjæt";
...
/**
 * 5.5: Helper-funktion til at hente en brugers abonnementsdata
 */
